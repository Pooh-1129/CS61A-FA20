def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item] + member for member in nested_list]
...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        lists = subseqs(s[1:])
        return lists + insert_into_all(s[0], lists)
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return insert_into_all(s[0],b) + a #是否取这一位
...
def insert(link, value, index):
    ...
    IndexError
    """
    if index == 0 and link.rest != Link.empty:
