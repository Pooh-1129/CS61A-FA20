...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    flag = 0
    table = [dice() for i in range(num_rolls)]
    for i in table:
        if i==1:
            flag = 1
    return 1 if flag else sum(table)
    # END PROBLEM 1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while True:
        if who == 0:
            ...
            up0 = take_turn(num0, score1, dice)
            score0 += up0
            ...
            up1 = take_turn(num1, score0, dice)
            score1 += up1
            say = say(score0, score1)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if j - last_score > running_high:
            print(j - last_score,"point(s)! The most yet for Player", who)
            return announce_highest(who, j, j - last_score)
        return announce_highest(who, j, running_high)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def try_average(*args):
        num = [original_function(*args) for i in range (trials_count)]
        ave = sum(num) / trials_count
        return ave
    return try_average
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    l = [func(i, dice) for i in range (1,11)]
    ans = l.index(max(l)) + 1 
    return ans
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    rolls 0 dice if it gives at least CUTOFF points and does not give an else num_rolls extra turn.
    Otherwise, it rolls NUM_ROLLS.
    """
    # BEGIN PROBLEM 11
    s = free_bacon(opponent_score)
    if extra_turn(score+s, opponent_score):
       return 0
