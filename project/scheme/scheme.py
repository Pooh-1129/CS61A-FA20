...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        lam_exp = do_lambda_form(Pair(target.rest, expressions.rest), env)
        env.define(target.first,lam_exp)
        return target.first
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        c = scheme_eval(expressions.first, env)
        if is_true_primitive(c):
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        name = target.first # expression.first.first
        formals = target.rest
