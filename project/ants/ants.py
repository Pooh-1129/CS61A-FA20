...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            old_contain_new = isinstance(old_ant, ContainerAnt) and old_ant.can_contain(self)
            new_contain_old = isinstance(self, ContainerAnt) and self.can_contain(old_ant) 
            
            if old_contain_new:
                old_ant.contain_ant(self)
                place.ant = old_ant
            elif new_contain_old:
                self.contain_ant(old_ant)
                place.ant = self
            else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
       #
        #Ant.reduce_armor(self,amount) 先攻击fireant可能会这只被removel
        if amount >=  self.armor:
            cnt = amount + self.damage
        else:
            cnt = amount
...
# END Problem EC
    ...
    implemented = True   # Change to True to view in the GUI
    # END Problem EC
    true_flag = True #share 的共有属性

    def __init__(self, armor=1):
        # BEGIN Problem EC
        self.fake = 0
        "*** YOUR CODE HERE ***"
        if QueenAnt.true_flag == False:
            self.fake = 1
        QueenAnt.true_flag = False
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Ant.reduce_armor(self,amount)
        if self.armor <= 0 and self.fake == 0:
            bees_win()
        # END Problem EC
...
def make_slow(action, bee):
    ...
    "*** YOUR CODE HERE ***"
    def new_action(gamestatus):
        if gamestatus.time %  2 == 0:
            action(gamestatus)
    return new_action
