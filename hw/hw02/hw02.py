...
def missing_digits(n):
    ...
    t1 = n%10
    t2 = (n//10)%10 
    return missing_digits(n//10) + max((t1-t2-1),0)
...
def count_coins(total):
    ...
    """
    "*** YOUR CODE HERE ***"
#注意coin可能是none加法就会有问题
    def cur(now, coin):
        if coin == None or now + coin > total:
            return 0
        elif now + coin == total:
            return 1
        return cur(coin + now, coin) + cur(now, next_largest_coin(coin))
